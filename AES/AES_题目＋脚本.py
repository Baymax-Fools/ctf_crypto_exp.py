...
def mix_columns_inv(grid):
    ...
    for i in range(0, 16, 4):
        grid[i:i + 4] = mix_column_inv(grid[i:i + 4])
...
def decrypt(b, expanded_key):
    # First round (reverse of final encryption round)
    add_round_key(b, expanded_key[-16:])
    shift_rows_inv(b)
    sub_bytes_inv(b)
