...
def brute_force():
    # 原始字符串模式，包含3个需要爆破的未知大写字母
    original_pattern = "TASC?O3RJMV?WDJKX?ZM"
    # 目标MD5模式，用于匹配验证
    target_md5_pattern = "E903???4DAB????08?????51?80??8A?"

    # 所有可能的大写字母
    uppercase_letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'

    # 生成所有可能的3个大写字母组合（26^3 = 17576种可能）
    for combo in itertools.product(uppercase_letters, repeat=3):
        # 用当前组合替换原始字符串中的问号
        test_string = original_pattern
        for char in combo:
            test_string = test_string.replace('?', char, 1)  # 每次替换一个问号

        # 计算当前字符串的MD5值
        test_md5 = md5(test_string)

        # 检查MD5值是否与目标模式匹配（对应位置相同或者是问号）
        if all(target_md5_pattern[i] in ('?', test_md5[i]) for i in range(len(target_md5_pattern))):
            print(f"原始字符串: {test_string}")
            print(f"32位MD5码: {test_md5}")
            print(f"flag: flag{{{test_md5}}}")
            return  # 找到匹配后立即退出
