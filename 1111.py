from Crypto.Util.Padding import pad
S = [
    
]

def xor(block):
    s=block
    for i in range(4):
        for j in range(4):
            block[i][j] ^= block[(i + 2) % 4][(j + 1) % 4]
    return s

def dexor(x):
    s=x
    for i in range(3,-1,-1):
        for j in range(3,-1,-1):
            s[i][j] ^= s[(i + 2) % 4][(j + 1) % 4]
    return s

c="f000ae7e94f48ef99da6390f99a08701cf16d63596bebac938ec36004d54b73d1712c2f38926c3bcc5e5f42c4d55b57ef1070a7b443677b3cc4372d9a41a4775"
c=int(c,16)
c_bytes = c.to_bytes((c.bit_length() + 7) // 8, 'big')
print(c_bytes)
print(len(c_bytes))

def rotate(row):
    row[0], row[1], row[2], row[3] = row[3], row[1], row[2], row[0]

def transpose(block):
    copyBlock = [[block[i][j] for j in range(4)] for i in range(4)]
    for i in range(4):
        for j in range(4):
            block[i][j] = copyBlock[j][i]
    return block

def deswap(x):
    s=0
    for i in range(4):
        for j in range(4):
            s += x[i][j]
    if s % 2:
        x = transpose(x)
    for i in range(2,-1,-1):
        for j in range(3,-1,-1):
            ii = ((x[i][j] & 0XFC) + i) % 4
            jj = (j + 3) % 4
            x[i][j], x[ii][jj] = x[ii][jj], x[i][j]
    rotate(x[3])
    rotate(x[2]);rotate(x[2]);rotate(x[2])
    rotate(x[1])
    rotate(x[0]);rotate(x[0])
    x[2], x[0] = x[0], x[2]
    x[1], x[2] = x[2], x[1]
    x[3], x[0] = x[3], x[0]
    x[1], x[0] = x[0], x[1]
    x[2], x[3] = x[3], x[2]
    x[2], x[0] = x[0], x[2]
    return x

def deadd(block):
    inv3 = 171  # modular inverse of 3 modulo 256, since 3 * 171 = 513 ≡ 1 (mod 256)

    # --- Row 0: each element was multiplied by 3 (x + 2*x)
    for j in range(4):
        block[0][j] = (block[0][j] * inv3) & 0xFF

    # --- Row 2: solve the 2-cycle within the row
    a1, b1, c1, d1 = block[2][0], block[2][1], block[2][2], block[2][3]
    # Solve: a1 = a + 2c, c1 = c + 2a  =>  c = (2*a1 - c1) * inv3 mod 256
    c = ((2 * a1 - c1) * inv3) & 0xFF
    a = (a1 - 2 * c) & 0xFF
    d = ((2 * b1 - d1) * inv3) & 0xFF
    b = (b1 - 2 * d) & 0xFF
    block[2][0], block[2][1], block[2][2], block[2][3] = a, b, c, d

    # --- Rows 1 and 3: four independent cross-pairs
    # Pair 1: (1,0) <-> (3,1)
    e1, n1 = block[1][0], block[3][1]
    e = ((2 * n1 - e1) * inv3) & 0xFF
    n = (n1 - 2 * e) & 0xFF
    block[1][0], block[3][1] = e, n

    # Pair 2: (1,1) <-> (3,2)
    f1, o1 = block[1][1], block[3][2]
    f = ((2 * o1 - f1) * inv3) & 0xFF
    o = (o1 - 2 * f) & 0xFF
    block[1][1], block[3][2] = f, o

    # Pair 3: (1,2) <-> (3,3)
    g1, p1 = block[1][2], block[3][3]
    g = ((2 * p1 - g1) * inv3) & 0xFF
    p = (p1 - 2 * g) & 0xFF
    block[1][2], block[3][3] = g, p

    # Pair 4: (1,3) <-> (3,0)
    h1, m1 = block[1][3], block[3][0]
    h = ((2 * m1 - h1) * inv3) & 0xFF
    m = (m1 - 2 * h) & 0xFF
    block[1][3], block[3][0] = h, m

    return block

def sub(x):
    for i in range(4):
        for j in range(4):
            x[i][j] = S[x[i][j]]
    return x

def desub(x):
    for i in range(4):
        for j in range(4):
            for y in range(len(S)):
                if x[i][j] == S[y]:
                    x[i][j] = y
                    break
    return x

def deround(x):
    dxor = dexor(x)
    dswa = deswap(dxor)
    dadd = deadd(dswa)
    dsub = desub(dadd)
    return dsub

def de(mat):
    x = [[mat[i * 4 + j] for j in range(4)] for i in range(4)]
    for _ in range(77):
        x = deround(x)
    return bytes([x[i][j] for i in range(4) for j in range(4)])

m = []
for i in range(0, len(c_bytes), 16):
    m += de(c_bytes[i : i + 16])

print(bytes(m))